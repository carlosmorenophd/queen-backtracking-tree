...
class Chess {
  ...
  createTree() {
    ...
    let i = 0;
    while (continueFlag) {
      const children = this.treeQueen.filter((item) =>
        parentNow.includes(item.parent)
      );
  ...
  addPush(newPosition) {
    const index = this.treeQueen.findIndex(
      (item) => item.id === newPosition.id
    );
    if (index >= 0) {
  ...
  isSquareThreatened(row, col) {
    return (
      this.isColumnThreatened(row, col) ||
